...
class Bot_chat:
    ...
    def best_pics(self, pics: list, pics_number):
        if len(pics) <= pics_number:
            return pics
        else:
            worst_pic_num = 0
        ...
        return self.best_pics(pics, pics_number)
    ...
    def start_bot(self, person, user_token, db_init):
        # id для таблиц candidates и photos для БД,
        # чтобы делать любое кол-во записей в пределах сессии
        last_photo_id = 0
        last_pers_id = 0
        pers_num = 1
        ...
        for event in self.longpoll.listen():
            ...
            for num, item in enumerate(candidates):
                try:
                    ...
                    best_pics = self.best_pics(photos['response']['items'], PICS_NUM)
                    last_photo_id = vk_db.photo_tab(item['id'], best_pics, pers_num + num, last_photo_id + 1, db_init)
                    for j in range(PICS_NUM):
                        try:
